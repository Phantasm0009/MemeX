...
async function loadMarketData() {
  try {
    ...
    console.error('Failed to load market data from backend');
  }
...
async function loadMetaData() {
  try {
    const meta = await fetchFromBackend('/api/meta');
    if (meta) return meta;
  } catch (error) {
    console.error('Failed to load meta data from backend');
  }
...
async function loadDatabase() {
  try {
    const database = await fetchFromBackend('/api/database');
...
// Instead of watching local files, periodically update from backend API
setInterval(async () => {
  try {
    const market = await fetchFromBackend('/api/market');
    if (market) {
